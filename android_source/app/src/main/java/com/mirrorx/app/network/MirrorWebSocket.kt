...
class MirrorWebSocket {
    ...
    data class AdaptInfo(
        val mode: String = "auto",    // "auto" | "manual" | "reduced" | "boosted"
        val reason: String = "",
        val quality: Int = 0,
        val scale: Int = 0,
        val fps: Float = 0f
    ...
    fun connect(host: String, port: Int = 9900) {
        ...
        ws = client!!.newWebSocket(request, object : WebSocketListener() {
            ...
            override fun onMessage(webSocket: WebSocket, text: String) {
                // JSON messages: screen_info, adapt, pong, hello (latency)
                try {
                    ...
                    when (json.optString("type")) {
                        ...
                        "hello" -> {
                            // v1.5.9: server echoes back heartbeat ms
                            if (json.has("ms")) {
                                _latencyMs.value = json.optInt("ms", 0)
                            }
                        }
                        "adapt" -> {
                            // v1.2.2: server is auto-adapting quality/scale.
            ...
            override fun onMessage(webSocket: WebSocket, bytes: okio.ByteString) {
                ...
                try {
                    ...
                    if (bitmap != null) {
                        ...
                        // v1.6.0: also update remoteCursor so TouchpadView shows PC cursor
                        _remoteCursor.value = Pair(normX, normY)
    ...
    /** v1.2: ask the server to hide the PC mouse cursor (stylus mode). */
